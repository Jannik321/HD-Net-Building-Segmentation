...
def criterion(inputs, target, loss_weight=None, dice: bool = True):
    if loss_weight is None:
        loss_weight = torch.tensor(1.0, device=inputs.device, dtype=inputs.dtype)
    else:
        loss_weight = loss_weight.to(device=inputs.device, dtype=inputs.dtype)
    ...
    # 自动把 target 对齐到 inputs 的空间尺寸
    if inputs.shape[2:] != target.shape[2:]:
        target = F.interpolate(target.float(), size=inputs.shape[2:], mode='nearest')
    else:
        target = target.float()

    loss = bcecriterion(inputs, target)

    if dice:
        loss += dice_loss_func(torch.sigmoid(inputs), target)
...
def train_net(read_name,
    ...
    # traindataset = BuildingDataset(dataset_dir=data_path, training=True, txt_name="final_train_10p.txt", data_name=args.DataSet) # 部分数据训练
    ...
    for epoch in range(epochs):
        ...
        with tqdm(total=len(traindataset), desc=f'Epoch {epoch + 1}/{epochs}', unit='img') as pbar:
            for num, batch in enumerate(train_loader):
                ...
                x_seg, x_bd, seg1, seg2, seg3, seg4, seg5, seg6, bd1_u, bd2_u, bd3_u, bd4_u, bd5_u, bd6_u = net(imgs)

                # Mass: 3 / 9 WHU: 7 / 21 Inria: 10 / 30
                loss = criterion(x_seg, true_labels, dice=True) + \
                    0.3 * criterion(seg1, true_labels, dice=True) + \
                    0.3 * criterion(seg2, true_labels, dice=True) + \
                    0.5 * criterion(seg3, true_labels, dice=True) + \
                    0.5 * criterion(seg4, true_labels, dice=True) + \
                    0.5 * criterion(seg5, true_labels, dice=True) + \
                    0.5 * criterion(seg6, true_labels, dice=True) + \
                    criterion(x_bd, edge_masks, loss_weight=torch.tensor(9.0), dice=True) + \
                    0.3 * criterion(bd1_u, edge_masks, loss_weight=torch.tensor(3.0), dice=True) + \
                    0.3 * criterion(bd2_u, edge_masks, loss_weight=torch.tensor(3.0), dice=True) + \
                    0.5 * criterion(bd3_u, edge_masks, loss_weight=torch.tensor(3.0), dice=True) + \
                    0.5 * criterion(bd4_u, edge_masks, loss_weight=torch.tensor(3.0), dice=True) + \
                    0.5 * criterion(bd5_u, edge_masks, loss_weight=torch.tensor(3.0), dice=True) + \
                    0.5 * criterion(bd6_u, edge_masks, loss_weight=torch.tensor(9.0), dice=True)
            ...
            with open(results_file, "a") as f:
                info = (
                    ...
                    f"val_IoU: {val_score:.6f}\n"
                    f"best_IoU: {best_val_score:.6f}\n"
                    f"lr: {cur_lr:.3e}\n"
                    f"epoch_time_sec: {epoch_time:.1f}\n"
                )
                f.write(info + "\n\n")
            # --- END NEW ---
            ...
            # ===== NEW: 每个 epoch 都保存 last（覆盖写，便于断点续训）=====
            # torch.save(net.module.state_dict(),
            # dir_checkpoint + save_name + '_last.pth')
